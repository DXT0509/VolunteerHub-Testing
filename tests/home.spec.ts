...
test.describe('Home page', () => {
  ...
  test('shows volunteer needs with mocked backend and SEE ALL button', async ({ page }) => {
    const mock = {
      hot_events: [
        {
          id: 1,
    ...
    await expect(page.locator('text=Volunteer Needs Now')).toBeVisible({ timeout: 5000 });
  ...
  test('contact form fills and submits without navigating', async ({ page }) => {
    ...
    await page.addInitScript(() => {
      ...
      // @ts-ignore
      window.location = { href: '', assign: () => {}, replace: () => {} } as any;
    });

    // Provide empty dashboard data to avoid noise
    await page.route('http://localhost:4000/dashboard', (route) =>
      route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ hot_events: [] }) })
    );
