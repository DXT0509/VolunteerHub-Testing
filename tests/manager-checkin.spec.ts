...
const CHECKOUT_ROUTE = '/check-out-volunteer';
...
test.describe('Manager - Check Out Volunteers', () => {
	...
	test('paginates registrations and exposes aria labels', async ({ page }) => {
		const registrations = Array.from({ length: 6 }, (_, index) =>
			buildRegistration({
				id: 100 + index,
				user: buildUser({ id: 700 + index, full_name: `Volunteer ${index + 1}` }),
			})
		);
		...
		await expect(page.getByRole('cell', { name: 'Volunteer 6' })).toBeVisible();
	});
