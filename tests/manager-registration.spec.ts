...
const PENDING_ROUTE = 'http://localhost:5173/manage-pending-registrations';
...
const mockPendingRegistrations = async (page: Page, getRegs: () => unknown[]) => {
	await page.route(`${API_BASE}/registrations/pending`, (route) => {
		if (route.request().method() !== 'GET') return route.continue();
		return route.fulfill({
			status: 200,
			contentType: 'application/json',
...
const mockPendingRegistrationsWithDelay = async (page: Page, getRegs: () => unknown[]) => {
	let release: () => void = () => {};
	const waitForRelease = new Promise<void>((resolve) => {
		release = resolve;
	});

	await page.route(`${API_BASE}/registrations/pending`, async (route) => {
		if (route.request().method() !== 'GET') return route.continue();
...
const mockUpdateRegistrationStatus = async (
	page: Page,
...
const setupManagerPendingPage = async (page: Page, user = managerUser) => {
	await seedAuth(page, user);
	await mockEvents(page, () => baseEvents);
};

test.describe('Manager - Pending registrations', () => {
	test('renders heading, loading state, and table content', async ({ page }) => {
		await setupManagerPendingPage(page);
		const { release } = await mockPendingRegistrationsWithDelay(page, () => [buildRegistration()]);

		await page.goto(PENDING_ROUTE);

		await expect(page.getByRole('heading', { name: 'Approve registrations' })).toBeVisible();
		await expect(page.getByText('View, approve or reject registrations for your campaigns.')).toBeVisible();
	...
	test('supports campaign filter and total count', async ({ page }) => {
		...
		await mockPendingRegistrations(page, () => registrations);
	...
	test('paginates registrations and exposes aria labels', async ({ page }) => {
		...
		await expect(page.getByText(/Page 2 \/ [2-9]\d*/)).toBeVisible();
		await expect(page.getByRole('cell', { name: 'user6' })).toBeVisible();
	});

	test('shows empty state when no pending registrations', async ({ page }) => {
		await setupManagerPendingPage(page);
		await mockPendingRegistrations(page, () => []);
	...
	test('shows error state when fetch fails', async ({ page }) => {
		...
		await expect(page.getByText('Failed to fetch registrations')).toBeVisible();
	});

	test('opens event detail dialog and closes with Escape', async ({ page }) => {
		await setupManagerPendingPage(page);
		await mockPendingRegistrations(page, () => [buildRegistration()]);
	...
	test('approves a registration and shows success toast', async ({ page }) => {
		await setupManagerPendingPage(page);
		await mockPendingRegistrations(page, () => [buildRegistration({ id: 77 })]);
		await mockUpdateRegistrationStatus(page, { status: 200, body: { success: true } });

		await page.goto(PENDING_ROUTE);
		await page.getByRole('button', { name: 'Approve' }).click();

		await expect(page.getByRole('alert')).toContainText('Approved registration');
	});

	test('shows warning toast when rejecting fails', async ({ page }) => {
		await setupManagerPendingPage(page);
		await mockPendingRegistrations(page, () => [buildRegistration({ id: 88 })]);
		await mockUpdateRegistrationStatus(page, { status: 400, body: { error: 'Reject failed' } });

		await page.goto(PENDING_ROUTE);
		await page.getByRole('button', { name: 'Reject' }).click();

		await expect(page.getByRole('alert')).toContainText('Reject failed');
	});
...
test.describe('Manager - Pending registrations navigation guards', () => {
