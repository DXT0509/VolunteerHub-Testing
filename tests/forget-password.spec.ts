...
test.describe("Forget Password – Stage 1: Verify", () => {
  ...
  test("User bị khóa: không cho phép reset password", async ({
    page,
  }) => {
...
test.describe("Forget Password – Stage 2: Reset Password", () => {
  ...
  // ---- Helpers ----
  async function openResetForm(page: any) {
  ...
  test("Nhập mật khẩu mới giống hệt mật khẩu cũ", async ({
    page,
  }) => {
    await openResetForm(page);
